...
const Account = () => {
  ...
  const handleSendResetLink = async () => {
    ...
    try {
      const response = await axios.post(`${import.meta.env.VITE_REACT_APP_URL}/api/v1/auth/send-reset-link`, {
    ...
    <div className="account">
      <div className="accountDetails">
        <div className="userData">
          <h3>Welcome 🎉 {auth.user.name}</h3>
          <h4>{auth.user.email}</h4>
          <button onClick={handleSendResetLink} disabled={isSendingLink}>
            {isSendingLink ? "Sending..." : "Change Your Password"}
          </button>
        </div>
        <div className="user">
          <img className="userImg" src="/img/user.png" alt="user" />
